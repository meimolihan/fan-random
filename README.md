...
+ [x] 支持 WebP / JPG / PNG / GIF 格式
+ [x] `?type=json` 返回 JSON（含图片 URL、类型、总数）
+ [x] 内置 836 张壁纸（pc 桌面端 418 张 + mp 移动端 418 张）
+ [x] 零第三方运行时依赖（仅 Node.js 内置模块）
+ [x] 图片列表内存缓存 + 刷新直接出图：首次刷新即可拿到新壁纸，无需等待二次请求
+ [x] 静态图片 URL immutable 长缓存：缓存命中后重复图片直接从本地加载
...
> ⚡ **性能设计**：随机端点 `/` `/pc` `/mp` 直接返回图片内容（200，无需客户端跟随跳转）。真实图片文件请求（`/pc/pc-117.webp`）以 `Cache-Control: immutable` 一次性交付，浏览器缓存后，反复刷新切换壁纸时多数图片直接从本地缓存加载，几乎零等待。新增图片会自动出现在随机池中（目录变化自动重扫缓存），无需手动干预。
...
1. **启动时**：HTTP 服务器扫描 `public/` 目录建立图片列表缓存（目录 mtime 变化时自动重扫，无需重启）
2. **请求时**：从内存缓存随机选取一张图片，**直接返回图片内容（200）**，无需跳转等待
3. **图片响应**：`/pc/*.webp` 等静态图片以 `Cache-Control: immutable` 长缓存交付，浏览器缓存后重复图片直接取本地，刷新切换近乎瞬时
4. **更新图片**：放入/删除新图片后目录时间戳变化，下次请求自动重新扫描，**无需重启服务**

## 🖼️ 添加/替换图片

> ⚡ **动态生效**：所有部署方式均支持运行时动态扫描图片。新增/删除图片后目录时间戳变化，服务会自动重扫缓存，**无需重启服务**。若希望替换同名图片后浏览器不再使用旧缓存，建议使用新文件名。
